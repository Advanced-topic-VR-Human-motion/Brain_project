...
{   
    //Split at 456
    public GameObject cathTop, cathTL, cathTR, cathBL, cathBR, catheder,
                    skullTL, skullTR, skullBL, skullBR, skullBrow; //references to marker spheres
    [SerializeField] Transform cathederTip;
    float timeToCall;
    float timeDelay = 0f; //the code will be run every 2 seconds
    ...
    bool readyToUpdate;
    LineRenderer lr;
    [SerializeField] int framesBetweenArrow;
    [SerializeField] bool firstHalf;
    int halfPoint = 456;
    [SerializeField] GameObject ArrowPrefab;
    List<Vector3> pathDataVectors = new List<Vector3>();
    List<Quaternion> pathDataQuaternions = new List<Quaternion>();
    ...
    void Start()
    {
        ...
        //close reader
        sr.Close();
        for (int i = (firstHalf ? 0 : halfPoint); i < (firstHalf ? halfPoint : fileSize); i++)
        {
            if (cathTip[i, 0] != 0 || cathTip[i, 1] != 0 || cathTip[i, 2] != 0)
            {
                catheder.transform.position = new Vector3(cathTip[i, 0] / 1000.0f, cathTip[i, 1] / 1000.0f, cathTip[i, 2] / 1000.0f);
                catheder.transform.LookAt(new Vector3(cathBottomLeft[i, 0] / 1000.0f, cathBottomLeft[i, 1] / 1000.0f, cathBottomLeft[i, 2] / 1000.0f), new Vector3(cathBottomRight[i, 0] / 1000.0f, cathBottomRight[i, 1] / 1000.0f, cathBottomRight[i, 2] / 1000.0f));
                pathDataVectors.Add(cathederTip.position);
                pathDataQuaternions.Add(cathederTip.rotation);
            }
        }
        TubeRenderer tr = GetComponent<TubeRenderer>();
        tr.SetPositions(pathDataVectors.ToArray());

        if (framesBetweenArrow < 0) framesBetweenArrow = 0;

        for (int i = 0; i < pathDataVectors.Count; i+=framesBetweenArrow+1)
        {
            Instantiate(ArrowPrefab, pathDataVectors[i], pathDataQuaternions[i]);
        }
    }
    ...
    void FixedUpdate()
    {
        if (Time.fixedTime >= timeToCall && MarkerCheck() && fileSize > 0 && readyToUpdate)
        {
            ...
            catheder.transform.position = new Vector3(x1, y1, z1);
            catheder.transform.LookAt(new Vector3(x4, y4, z4), new Vector3(x5, y5, z5));

            index++;
            if (index >= (firstHalf ? halfPoint : fileSize)) index = firstHalf ? 0 : halfPoint;//readyToUpdate = false; //stop simulation if eod is reached
