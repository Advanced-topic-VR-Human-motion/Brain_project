...
public class FABRIKEffector : MonoBehaviour
{
    ...
    [HideInInspector]
    public float swingUpDownConstraint = float.NaN;
    [HideInInspector]
    public float swingLeftRightConstraint = float.NaN;
    ...
    public float SwingUpDownConstraint
    {
        get
        {
            return swingUpDownConstraint * 0.5F * Mathf.Deg2Rad;
        }
    }

    public float SwingLeftRightConstraint
    {
        get
        {
            return swingLeftRightConstraint * 0.5F * Mathf.Deg2Rad;
        }
    }
    ...
    public bool SwingUpDownConstrained
    {
        get
        {
            return !float.IsNaN(swingUpDownConstraint);
        }
    }

    public bool SwingLeftRightConstrained
    {
        get
        {
            return !float.IsNaN(swingLeftRightConstraint);
        }
    }
    ...
    public void ApplyConstraints(Vector3 direction)
    {
        ...
        if (parent)
        {
            ...
            if (!SwingUpDownConstrained && !SwingLeftRightConstrained && !TwistConstrained)
            {
            ...
            else
            {
                ...
                if (SwingUpDownConstrained){
                    swing = swing.ConstrainUpDown(SwingUpDownConstraint);
                }
                if (SwingLeftRightConstrained)
                {
                    swing = swing.ConstrainLeftRight(SwingLeftRightConstraint);
                }
                if (TwistConstrained)
                {
                ...
                Rotation = Quaternion.Slerp(Rotation, rotation_global * swing * twist, lerpFactor);
            }
        }
        //if this is the root effector, we don't need to apply any constraints
        else
        {
